...
class SpeedTestTool:
    ...
    def __init__(self, root):
        ...
        self.test_servers = [
            ...
            {"name": "公共DNS", "host": "8.8.8.8", "port": 53},
    ...
    def create_widgets(self):
        ...
        # 数据大小
        ttk.Label(config_frame, text="数据大小(KB):").grid(row=0, column=4, padx=(0, 5), sticky=tk.W)
        self.size_var = tk.StringVar(value="1024")
        ttk.Entry(config_frame, textvariable=self.size_var, width=10).grid(row=0, column=5)
    ...
    def start_test(self):
        ...
        try:
            ...
            data_size = int(self.size_var.get()) * 1024  # 转换为字节
        except ValueError:
        ...
        self.result_text.insert(tk.END, f"数据大小: {data_size // 1024} KB\n")
        self.result_text.insert(tk.END, "=" * 50 + "\n\n")

        # 启动测试线程
        thread = threading.Thread(target=self.run_test, args=(server, test_count, data_size))
        thread.daemon = True
        thread.start()

    def run_test(self, server, test_count, data_size):
        """运行测试"""
        try:
            # 测试延迟
            self.root.after(0, self.update_status, "正在测试延迟...")
            ping_results = self.test_ping(server["host"], test_count)
            avg_ping = statistics.mean(ping_results)
            self.root.after(0, self.show_ping_result, ping_results, avg_ping)

            # 测试下载速度
            self.root.after(0, self.update_status, "正在测试下载速度...")
            download_results = self.test_download(server["host"], server["port"], test_count, data_size)
            avg_download = statistics.mean(download_results)
            self.root.after(0, self.show_download_result, download_results, avg_download)

            # 测试上传速度
            self.root.after(0, self.update_status, "正在测试上传速度...")
            upload_results = self.test_upload(server["host"], server["port"], test_count, data_size)
            avg_upload = statistics.mean(upload_results)
            self.root.after(0, self.show_upload_result, upload_results, avg_upload)
    ...
    def test_download(self, host, port, count, data_size):
        """测试下载速度"""
        results = []
        for i in range(count):
            try:
    ...
    def test_upload(self, host, port, count, data_size):
        """测试上传速度"""
        results = []
        for i in range(count):
            try:
    ...
    def show_ping_result(self, results, avg):
        ...
        self.result_text.insert(tk.END, f"  丢包率: {results.count(999) / len(results) * 100:.1f}%\n\n")

    def show_download_result(self, results, avg):
        """显示下载结果"""
        self.result_text.insert(tk.END, "【下载速度】\n")
        self.result_text.insert(tk.END, f"  平均速度: {avg:.2f} MB/s\n")
    ...
    def load_history(self):
        """加载历史记录"""
        history_file = os.path.join(os.path.dirname(__file__), "speed_test_history.json")
